...
- `model.js` definisce invarianti, normalizza ogni input, migra il formato organizer v1 al formato v2 e valida l'estensione v3 con release plan v1-v6.
...
- **Piano sequenziale:** un modulo inizia dopo la fine del precedente; gli organizer usano gli slot, mentre un release plan v5/v6 può usare una capacità settimanale astratta senza fasce orarie.
- **Confronto esplicito:** un release plan v6 conserva il delta fra due snapshot per sezione e per elemento; la UI lo presenta senza inferirlo dalla cronologia testuale.
